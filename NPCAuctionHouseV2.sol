...
contract NPCAuctionHouseV2 is INPCAuctionHouse, UUPSUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable, OwnableUpgradeable {
    ...
    function _createAuction() internal {
        try npc.mint() returns (uint256 npcId) {
            ...
            if(lastAuction >= 10){
                uint256 price = 0;
                for(uint256 i = lastAuction; i > lastAuction - 10; i--){
                    price = price + _pastAuctionAmounts[i];
                }

                initialBid = price / 10;
                uint256 percentage = (initialBid * 330) / 10000;
